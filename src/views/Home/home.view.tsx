...
function Home() {
    ...
    const [pb, devices] = usePlaybackStore(
        (state) => [state.playback, state.devices],
        shallow
    );
    ...
    return (
        <>
            ...
            >
                <div
                    className={`relative rounded-md mb-2 col-span-3 lg:col-span-2 xl:col-span-1 bg-transparent`}
                >
                    <Sidebar />
                </div>
                <div
                    className={`bg-gradient-to-tr from-grack-900 to-black  relative col-span-full col-start-4 lg:col-start-3 xl:col-start-2 ml-2 mb-2 overflow-x-auto rounded-md `}
                >
                    <Header />
                    {location.pathname === "/" && (
                ...
                <div className="flex items-center justify-end h-[24px] px-2 text-xs font-semibold  text-black bg-spotify justify-right col-span-full mt-2">
                    <p className="flex items-center gap-2 mr-4">
                        ...
                        {devices?.devices?.map((device) => device.is_active && device.name)}
                    </p>
                </div>
            </main>
        </>
